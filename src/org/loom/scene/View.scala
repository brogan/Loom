/**
View the notional view with 0,0,0 at the centre.
Translates from view coordinates to screen coordinates and vice versa.
Remember, the screen 0,0 is at the top left and y increases downwards.
...
class View(val screenWidth: Int, val screenHeight: Int, val viewWidth: Int, val viewHeight: Int, val borderX: Int, val borderY: Int) {

   val viewCenterX:Int = viewWidth/2
   val viewCenterY:Int = viewHeight/2
   
   /**
   Converts a 2D view point to a drawing panel coordinate
   @param v input view point (Vector2D)
   @return Vector2D
   */
   def viewToScreenVertex(v: Vector2D): Vector2D = {
      val x: Double = borderX + viewCenterX + v.x
      val y: Double = borderY + viewCenterY - v.y
      new Vector2D(x, y)
   }
   /**
   Converts a drawing panel coordinate to a 2D view point
   @param v input drawing panel coordinate (Vector2D)
   @return Vector2D
   */
   def screenToViewVertex(v: Vector2D): Vector2D = {
